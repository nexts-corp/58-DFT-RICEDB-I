...
function insertData($val){
    ...
    if($dataCk["Num"] == 0){*/
    $sqlIns = "INSERT INTO dft_Rice_Tracking_V2(Code, Bag_No, LK_Province_Id, LK_Project_Id, Round, Silo, Address, LK_Associate_Id, LK_Type_Id, Warehouse, Stack, Weight, Sampling_Id, LK_Grade_Id, Discount_Rate, Weight_All)"
...
function updateAddr(){
    ...
    $sqlUp = "UPDATE dft_Rice_Tracking_V2 SET Address=grouped.Address
                FROM (Select Silo, Address FROM
                    dft_Rice_Tracking_V2
                    WHERE Address != ''
                GROUP BY Silo, Address) grouped
              WHERE dft_Rice_Tracking_V2.Silo=grouped.Silo";
    if($resIns = $link -> query($sqlUp)){
        print $sqlUp."<br>";
    }

    $sqlUp2 = "UPDATE dft_Rice_Tracking_V2 SET LK_Status_Keyword='AU4/2558'";

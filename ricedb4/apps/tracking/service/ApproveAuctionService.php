...
//                . " order by province,associateId,wareHouseCode";
        $sql = "select a.auctionNo,a.bidderNo,a.bidderAuctionNo,a.bidderQueue,a.bidderRound,a.bidderName,a.bidderWinner, "
                . " a.bidderPrice,a.bidderFirstPrice,a.bidderLastPrice,a.RFV2, "
                . " a.wareHouseCode,a.oweightAll, "
                . " a.provinceId,a.province,a.associateId,a.associate,a.isSale, "
                . "a.bidderPriceNo, "
                . "a.remarkPrice as remark "
                ...
                // . " where a.bidderWinner= 'Y' and (a.isSale is null or a.isSale = 'N') "
                . " where a.bidderMaxPrice = 'Y' and a.isSale is null "
...
//                . " inner join dft_Bidder_Price_Silo c"
//                . " on c.bidder_item_id = b.Id"
//                . " where a.bidderWinner= 'Y' and a.isSale  "
//                . " order by a.province,a.associateId,a.wareHouseCode";
        $sql = "select a.auctionNo,a.bidderNo,a.bidderAuctionNo,a.bidderQueue,a.bidderRound,a.bidderName,a.bidderWinner, "
                . " a.bidderPrice,a.bidderFirstPrice,a.bidderLastPrice,a.RFV2, "
                . " a.wareHouseCode,a.oweightAll, "
                ...
                . "a.remarkPrice as remark "
                . " from dbo.fn_auction_info('" . $auccode . "') a "
                . " where a.bidderMaxPrice = 'Y' and a.isSale is not null "
                . " order by a.province,a.associateId,a.wareHouseCode";
        return $this->datacontext->pdoQuery($sql);
    }
    ...
    public function update($data, $isSale, $remark, $auccode) {
        ...
        foreach ($data as $key => $value) {
            ...
            if ($this->datacontext->updateObject($result)) {
                ...
                $tracking->associateId = $value->associateId;
    ...
    public function delete($dataAuction) {
        ...
        $price->id = $dataAuction->bidderPriceNo;
